...
Item {
    ...
    // caelestia-assist launcher (assistant/bin/) is not on PATH.
    property string assistantRepoDir: ""

    // One JSON request on stdin, one JSON response on stdout — the
    // bridge contract (assistant/brain/bridge.py). Read-only ops.
    function assistantBridgeCommand(requestJson) {
        var payload = shellQuote(requestJson);
        var viaBin = "printf '%s' " + payload + " | caelestia-assist api";
        if (assistantRepoDir === "")
            return viaBin;
        return "command -v caelestia-assist >/dev/null 2>&1 && " + viaBin
            + " || printf '%s' " + payload + " | env PYTHONPATH="
            + shellQuote(assistantRepoDir) + " python3 -m assistant api";
    }

    // Compact renderer for a genius bridge response (mirrors the
    // CLI's _render_genius: scalar fields first, short lists, note
    // last; the bridge's error envelope is reported as an error).
    function renderGeniusBridgeResult(toolName, bridgeStdout, bridgeStderr) {
        var text = (bridgeStdout || "").trim();
        if (!text)
            return "Tool: " + toolName + "\nError: the assistant bridge produced no output"
                + (bridgeStderr && bridgeStderr.trim() ? " (" + bridgeStderr.trim().substring(0, 120) + ")" : "")
                + " — is caelestia-assist installed?" + "\n\n";
        var envelope;
        try {
            envelope = JSON.parse(text);
        } catch (e) {
            return "Tool: " + toolName + "\nError: unparseable bridge output: " + text.substring(0, 200) + "\n\n";
        }
        if (envelope.ok === false || envelope.error)
            return "Tool: " + toolName + "\nError: " + (envelope.error || "bridge call failed") + "\n\n";
        ...
        for (var key in res) {
            ...
            else if (Array.isArray(v) && v.length > 0 && v.length <= 8
                     && v.every(function (x) { return typeof x !== "object"; }))
                parts.push(key.replace(/_/g, " ") + ": " + v.join(", "));
        }
        var out = "Tool: " + toolName + "\nResult: " + parts.join("; ");
        if (typeof res.note === "string" && res.note)
            out += "\nNote: " + res.note;
        return out + "\n\n";
    }
    ...
    function handleAgentProcessResult(type, stdout, stderr, cmd) {
        if (type === "screenshot_take") {
            ...
            runningToolsCount--;
            checkToolsFinished();
        } else if (type.startsWith("genius_")) {
            // Bridge-backed genius tools: compact render, errors
            // stay errors — never swallowed, never a crash.
            accumulatedToolResults += renderGeniusBridgeResult(type.substring(7), stdout, stderr);
            runningToolsCount--;
            checkToolsFinished();
        }
    }
    ...
    // ---- Settings tool confirm flow (issue #120) --------------------------
    // A multi-op settings plan is surfaced as a chat card; nothing is
    // written until the user presses Apply (SettingsTools.confirmPlan).
    property bool settingsCardPending: false

    function showSettingsConfirmCard(plan) {
        // One pending card at a time: a newer request supersedes an older
        // unresolved card, which resolves as cancelled without a write.
        if (settingsCardPending)
            resolveSettingsCard(false, true);
        settingsCardPending = true;
        chatHistory.append({
            "isUser": false,
            "text": "",
            "isFinished": true,
    ...
    function resolveSettingsCard(applied, superseded = false) {
        if (!settingsCardPending)
            return;
        settingsCardPending = false;
        const res = applied ? SettingsTools.confirmPlan() : SettingsTools.cancelPlan();
        for (let i = chatHistory.count - 1; i >= 0; i--) {
            const m = chatHistory.get(i);
            if (m.isSettingsPlan && m.planResolved === "") {
                chatHistory.setProperty(i, "planResolved",
                    superseded ? "superseded" : (applied ? "applied" : "cancelled"));
                chatHistory.setProperty(i, "planResult",
                    superseded ? "Superseded by a newer request; nothing was written."
                               : (res.message || ""));
                break;
            }
        }
        saveHistory();
        accumulatedToolResults += "Tool: caelestia_setting_set\nResult: "
            + (superseded ? "Superseded by a newer request; nothing was written."
                          : (res.message || String(res.reason || ""))) + "\n\n";
        runningToolsCount--;
        checkToolsFinished();
    }
                            ...
                            if (enableTools) {
                                ...
                                for (var t = 0; t < textToolCalls.length; t++) {
                                    ...
                                    if (toolName === "take_screenshot") {
                                        ...
                                        currentActionText = "Listing settings tools...";
                                        var listRes = SettingsTools.listTools(args.group !== undefined ? String(args.group) : null);
                                        accumulatedToolResults += listRes.ok
                                            ? "Tool: caelestia_setting_list\nResult: groups: " + listRes.groups.join(", ") + "\n" + listRes.lines.join("\n") + "\n\n"
                                            : "Tool: caelestia_setting_list\nError: " + listRes.reason + "\n\n";

                                    } else if (toolName === "caelestia_setting_get") {
                                        currentActionText = "Reading setting...";
                                        var getRes = SettingsTools.get(String(args.path || ""));
                                        accumulatedToolResults += getRes.ok
                                            ? "Tool: caelestia_setting_get\nResult: " + getRes.path + " = " + getRes.value + " (" + getRes.kind + ", " + getRes.validation + ", default " + getRes.default + ")\n\n"
                                            : "Tool: caelestia_setting_get\nError: " + getRes.reason + "\n\n";

                                    } else if (toolName === "caelestia_setting_explain") {
                                        currentActionText = "Explaining setting...";
                                        var expRes = SettingsTools.explain(String(args.path || ""));
                                        accumulatedToolResults += expRes.ok
                                            ? "Tool: caelestia_setting_explain\nResult: " + expRes.answer + "\n\n"
                                            : "Tool: caelestia_setting_explain\nError: " + expRes.reason + "\n\n";

                                    } else if (toolName === "caelestia_setting_history") {
                                        currentActionText = "Reading undo history...";
                                        var hist = SettingsTools.historyEntries();
                                        var histLines = hist.map(function(e) {
                                            return "#" + e.id + " " + e.at + " — " + (e.label || e.ops + " change(s)") + " (" + e.ops + " change(s))";
                                        });
                                        accumulatedToolResults += "Tool: caelestia_setting_history\nResult: "
                                            + (histLines.length ? "\n" + histLines.join("\n") : "(empty)") + "\n\n";

                                    } else if (toolName === "caelestia_setting_undo") {
                                        currentActionText = "Undoing settings change...";
                                        var undoRes;
                                        if (args.id !== undefined)
                                            undoRes = SettingsTools.undoById(Number(args.id));
                                        else
                                            undoRes = SettingsTools.undo(args.steps !== undefined ? Number(args.steps) : 1);
                                        accumulatedToolResults += undoRes.ok
                                            ? "Tool: caelestia_setting_undo\nResult: " + undoRes.message + "\n\n"
                                            : "Tool: caelestia_setting_undo\nError: " + undoRes.reason + "\n\n";

                                    } else if (toolName === "caelestia_setting_set") {
                                        // Issue #120 confirmation policy: one op applies
                                        // directly (undoable); more than one op waits
                                        // for the user's Apply on the preview card.
                                        var setCalls = args.calls || [];
                                        var setRes = SettingsTools.request(setCalls, String(args.label || ""));
                                        if (setRes.needsConfirm) {
                                            runningToolsCount++; // resolved by the card buttons
                                            showSettingsConfirmCard(setRes.plan);
                                        } else {
                                            accumulatedToolResults += setRes.ok
                                                ? "Tool: caelestia_setting_set\nResult: " + setRes.message + "\n\n"
                                                : "Tool: caelestia_setting_set\nError: " + setRes.reason + "\n\n";
                                        }

                                    } else if (toolName === "caelestia_preset_list") {
                                        currentActionText = "Listing presets...";
                                        var pres = SettingsTools.presetList();
                                        var presLines = pres.map(function(p) {
                                            return p.name + " — " + p.description + " (" + p.calls + " changes)";
                                        });
                                        accumulatedToolResults += "Tool: caelestia_preset_list\nResult: \n" + presLines.join("\n") + "\n\n";

                                    } else if (toolName === "caelestia_preset_apply") {
                                        var presetRes = SettingsTools.requestPreset(String(args.name || ""));
                                        if (presetRes.needsConfirm) {
                                            runningToolsCount++; // resolved by the card buttons
                                            showSettingsConfirmCard(presetRes.plan);
                                        } else {
                                            accumulatedToolResults += presetRes.ok
                                                ? "Tool: caelestia_preset_apply\nResult: " + presetRes.message + "\n\n"
                                                : "Tool: caelestia_preset_apply\nError: " + presetRes.reason + "\n\n";
                                        }

                                    } else if (toolName === "caelestia_genius_math") {
                                        // Deterministic on-device compute through the assistant's JSON
                                        // bridge: the model builds the expression, the bridge evaluates it —
                                        // Ollama is never contacted (the op is allow-listed, read-only).
                                        currentActionText = "Computing...";
                                        runAgentCommand(assistantBridgeCommand(JSON.stringify({ op: "genius_math", expr: String(args.expr || "") })), "genius_" + toolName);
                                    } else if (toolName === "caelestia_genius_stats") {
                                        currentActionText = "Crunching numbers...";
                                        runAgentCommand(assistantBridgeCommand(JSON.stringify({ op: "genius_stats", numbers: Array.isArray(args.numbers) ? args.numbers : [] })), "genius_" + toolName);
                                    } else if (toolName === "caelestia_genius_logic") {
                                        currentActionText = "Checking logic...";
                                        runAgentCommand(assistantBridgeCommand(JSON.stringify({ op: "genius_logic", formula: String(args.formula || "") })), "genius_" + toolName);
                                    } else if (toolName === "caelestia_genius_decide") {
                                        currentActionText = "Comparing options...";
                                        runAgentCommand(assistantBridgeCommand(JSON.stringify({ op: "genius_decide", matrix: Array.isArray(args.matrix) ? args.matrix : [], labels: Array.isArray(args.labels) ? args.labels : [], criteria: Array.isArray(args.criteria) ? args.criteria : [], weights: Array.isArray(args.weights) ? args.weights : null, benefits: Array.isArray(args.benefits) ? args.benefits : null, method: String(args.method || "wsm") })), "genius_" + toolName);
                                    } else if (toolName === "caelestia_genius_palette") {
                                        // Contrast-check before suggesting: the palette op only
                                        // computes; any scheme/wallpaper change stays an inert
                                        // SUGGESTED_NOT_EXECUTED suggestion downstream.
                                        currentActionText = "Building palette...";
                                        runAgentCommand(assistantBridgeCommand(JSON.stringify({ op: "genius_palette", hex: String(args.hex || ""), harmony: String(args.harmony || "analogous"), n: Number(args.n) || 5 })), "genius_" + toolName);
             ...
             Item {
                 ...
                 VerticalFadeListView {
                     ...
                     delegate: Item {
                         ...
                         required property bool isSettingsPlan
                         ...
                         StyledRect {
                             ...
                             Column {
                                 ...
                                 // Settings plan confirm card (issue #120):
                                 // the preview-then-confirm flow for multi-op plans.
                                 Column {
                                     id: settingsPlanCard
                                     ...
                                     StyledText {
                                         width: parent.width
                                         wrapMode: Text.Wrap
                                         text: delegateItem.planResolved === ""
                                             ? (delegateItem.planLabel !== ""
                                                 ? qsTr("I found several changes that match your request (%1):").arg(delegateItem.planLabel)
                                                 : qsTr("I found several changes that match your request:"))
                                             : (delegateItem.planResolved === "applied"
                                                 ? qsTr("Applied:")
                                                 : (delegateItem.planResolved === "superseded"
                                                     ? qsTr("Superseded by a newer request:")
                                                     : qsTr("Cancelled — nothing was written:")))
                                         color: Colours.palette.m3onSurface
                                         font: Tokens.font.body.small
                                     }

                                     Repeater {
                                         model: delegateItem.planOps

                                         delegate: StyledText {
                                             required property var modelData
                                             width: settingsPlanCard.width
                                             wrapMode: Text.Wrap
                                             text: (modelData.noOp ? "· " : "– ")
                                                 + modelData.path + ": "
                                                 + modelData.from + " → " + modelData.to
                                                 + (modelData.noOp ? " (already set)" : "")
                                             color: modelData.noOp
                                     ...
                                     Row {
                                         visible: delegateItem.planResolved === ""
                                         spacing: Tokens.spacing.small
                                         ...
                                         TextButton {
                                             ...
                                             font: Tokens.font.body.small
                                             onClicked: root.resolveSettingsCard(true)
                                         }
                                         ...
                                         TextButton {
                                             ...
                                             font: Tokens.font.body.small
                                             onClicked: root.resolveSettingsCard(false)
                                         }
                                     }

                                     StyledText {
                                         visible: delegateItem.planResult !== ""
                                         width: parent.width
                                         wrapMode: Text.Wrap
                                         text: delegateItem.planResult
                                         color: Colours.palette.m3onSurfaceVariant
                                         font: Tokens.font.body.small
                                     }
                                 }
